...
double gVertVary[renVARYDIMBOUND * renVERTNUMBOUND];
...
void meshRender(meshMesh *mesh, renRenderer *ren, double unif[], texTexture *tex[]) {
	for (int i = 0; i < mesh->vertNum; i++) {
		ren->transformVertex(ren, unif, meshGetVertexPointer(mesh, i), &gVertVary[i * ren->varyDim]);
	}
	for (int i = 0; i < mesh->triNum; i++) {
		int *triangle = meshGetTrianglePointer(mesh, i);
		double *aVary = &(gVertVary[triangle[0] * ren->varyDim]);
		double *bVary = &(gVertVary[triangle[1] * ren->varyDim]);
		double *cVary = &(gVertVary[triangle[2] * ren->varyDim]);		
		clipRender(ren, unif, tex, aVary, bVary, cVary);
	}
	return;
}
