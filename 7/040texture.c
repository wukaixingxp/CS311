...
void texSample(texTexture *tex, double s, double t) {
	...
	if (tex->filtering == texQUADRATIC) {
		double uF = u - floor(u);
		double vF = v - floor(v);
		// calculate the contributions from the four pixels
		texGetTexel(tex, floor(u), floor(v), tex->aux);
		double leftDown[tex->texelDim];
		vecScale(tex->texelDim, (1 - uF) * (1 - vF), tex->aux, leftDown);

		texGetTexel(tex, ceil(u), floor(v), tex->aux);
		double rightDown[tex->texelDim];
		vecScale(tex->texelDim, (uF) * (1 - vF), tex->aux, rightDown);

		texGetTexel(tex, floor(u), ceil(v), tex->aux);
		double leftUp[tex->texelDim];
		vecScale(tex->texelDim, (1 - uF) * (vF), tex->aux, leftUp);

		texGetTexel(tex, ceil(u), ceil(v), tex->aux);
		double rightUp[tex->texelDim];
		vecScale(tex->texelDim, (uF) * (vF), tex->aux, rightUp);

		// add up to get the color of the pixel
		double rgb[tex->texelDim];
		vecAdd(tex->texelDim, leftDown, rightDown, rgb);
		vecAdd(tex->texelDim, rgb, leftUp, rgb);
		vecAdd(tex->texelDim, rgb, rightUp, rgb);

		// save the sum into sample
		vecCopy(tex->texelDim, rgb, tex->sample);
	} else
