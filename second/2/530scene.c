...
void sceneRender(sceneNode *node, GLdouble parent[4][4], GLint modelingLoc, 
	...
	if (node->firstChild != NULL) {
		sceneRender(node->firstChild, m, modelingLoc, unifNum, unifDims, unifLocs, attrNum, attrDims, attrLocs);
	}
	if (node->nextSibling != NULL) {
		sceneRender(node->nextSibling, parent, modelingLoc, unifNum, unifDims, unifLocs, attrNum, attrDims, attrLocs);
	}
}
